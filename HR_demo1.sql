...
(EMPLOYEES) 테이블에서 급여가 $7,000~$10,000 범위 이외인 
...
[3] 사원의 이름(last_name) 중에 ‘e’ 및 ‘o’ 글자가 포함된 사원을 출력하시오.
...
[4] HR 부서에서는 급여(salary)와 수당율(commission_pct)에 대한 지출 보고서를 작성하려고 한다. 이에 수당을 받는 모든 사원의 성과 이름(Name으로 별칭), 급여, 업무, 수당율, 연봉을 출력하시오. 이때 급여가 큰 순서대로 정렬하되, 급여가 같으면 수당율이 큰 순서대로, 연봉이 같으면 급여가 큰 순서대로 정렬하시오.

select concat(first_name,' ',last_name) as 'name' 
	, salary
    , job_id
    , commission_pct
    , salary*12*commission_pct as 'payment'
from employees
where commission_pct is not null
order by payment desc, salary desc;
